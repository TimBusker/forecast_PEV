...
print ('precip resolution is:'+ str(compute_res(precip)))
...
############################################# INTERPOLATE STEP ONE --> to .125 res #############################################

############# EFI ################
# get current efi resolution
print ('efi resolution is:'+ str(compute_res(efi_merged)))
# resample efi_merged to 0.125 degree resolution
efi_merged_int=efi_merged.interp_like(ref_grid, method='linear')
compute_res(efi_merged_int)

# save efi_merged as .grib and .nc file
efi_merged_int.to_netcdf(path_base+'/efi_merged_int.nc')


############# SOT ################
# get current efi resolution
print ('sot resolution is:'+ str(compute_res(sot_merged)))
# resample sot_merged to 0.125 degree resolution
sot_merged_int=sot_merged.interp_like(ref_grid, method='linear')
compute_res(sot_merged_int)

# save efi_merged as .grib and .nc file
sot_merged_int.to_netcdf(path_base+'/sot_merged_int.nc')


############# PRECIP ################
# select the >2000 period 
precip_TS=precip.sel(time=slice('2000-01-01', '2023-12-31'))

# precip interplike dummy 0.125 degree file 
precip_int=precip_TS.interp_like(ref_grid, method='linear')

# to dtype float32
#precip_int=precip_int.astype('float32')

precip_crop=precip.where(precip['latitude'] > efi_merged.latitude.min().values, drop=True)


# save precip_int to .nc
precip_int.to_netcdf(path_obs+'/precip_0125.nc')
...
mv_rainfall=mv.read(path_obs+'/precip_0125.nc') # load int res
p_netcdf= xr.open_dataset('/scistor/ivm/tbr910/precip_analysis/obs/precip_0125.nc') # load int res

regrid_rainfall = mv.regrid(
    grid          = [target_res,target_res],
    interpolation = "grid_box_average", #the spatially-weighted average of the source grid boxes that cover its area, meaning that total quantities are conserved within each grid box and over the entire data
    data          = mv_rainfall.to_dataset(),
    area          = 73.5/-27/33/45, #  (equivalent to area=E) --> North/West/South/East
    accuracy      = 40,
)

# repair regridded file 
regrid_rp=regrid_rainfall.to_dataset()
regrid_rp= regrid_rp.rename({'t':'rr'})
regrid_rp= regrid_rp.drop('valid_time').drop('surface').drop('step')
# drop attributes
regrid_rp.rr.attrs = {}
regrid_rp.latitude.attrs = {}
regrid_rp.longitude.attrs = {}


# check re-gridding
regrid_rp.isel(time=200).rr.plot.imshow(vmax=70)
p_netcdf.isel(time=200).rr.plot.imshow(vmax=70)
precip.sel(time=slice('2000-01-01', '2023-12-31')).isel(time=200).rr.plot()

# select denmark area 
precip_plot=precip.sel(time=slice('2000-01-01', '2023-12-31')).isel(time=200).rr
# get the highest rr value coordinates (the lat and lon of the highest value)
precip_plot.where(precip_plot==precip_plot.max(), drop=True).latitude.values
precip_plot.where(precip_plot==precip_plot.max(), drop=True).longitude.values

precip_2_plot=precip.sel(time=slice('2000-01-01', '2023-12-31')).isel(time=200).rr
# get the highest rr value coordinates (the lat and lon of the highest value)
precip_2_plot.where(precip_2_plot==precip_2_plot.max(), drop=True).latitude.values
precip_2_plot.where(precip_2_plot==precip_2_plot.max(), drop=True).longitude.values




# save regrid_rainfall to .nc 
regrid_rp.to_netcdf(path_obs+'/precip_%s.nc'%(str(target_res).replace('.','')))

print ('precip resolution is:'+ str(compute_res(regrid_rp)))
...
rainfall_025=xr.open_dataset(path_obs+'/rr_ens_spread_0.25deg_reg_v28.0e.nc')
rainfall_025=rainfall_025.sel(time=slice('2000-01-01', '2023-12-31'))
print ('rainfall resolution is:'+ str(compute_res(rainfall_025)))
